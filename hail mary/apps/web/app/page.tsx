...
import { Activity, AlertTriangle, CheckCircle, ArrowRight, Sparkles, Eye, EyeOff, Filter } from "lucide-react";
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="flex flex-col gap-5">
      ...
      <motion.div variants={itemVariants} className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-xl font-semibold tracking-tight">Lot Overview</h1>
          </div>
          <p className="text-[13px] text-muted-foreground/40 font-light">
            Cohort-relative anomaly scatter — {allComponents.length} components
          </p>
        </div>
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-4 gap-3">
        {[
          ...
          { label: "Rate", value: allComponents.length > 0 ? `${((flagged.length / allComponents.length) * 100).toFixed(1)}%` : "0%", icon: Filter, color: "oklch(0.6 0.10 300)" },
        ].map((stat) => (
        ...
        <div className="glass-card rounded-2xl p-6">
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-[10px] text-muted-foreground/35 mt-0.5 font-light">
                Leakage current vs propagation delay — cohort-relative view
              </p>
            </div>
          ...
          <div className="flex items-center gap-5 mb-3 text-[10px]">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-muted-foreground/40 font-light">Normal</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 rounded-full" style={{ background: "oklch(0.62 0.18 25)", border: "1.5px solid oklch(0.62 0.18 25 / 0.5)" }} />
              <span className="text-muted-foreground/40 font-light">Anomalous</span>
            </div>
            <span className="text-muted-foreground/20">|</span>
            <span className="text-muted-foreground/30 font-light">Hover to inspect · Click table row to drill in</span>
          </div>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              >
                ...
                <XAxis formatTick={(v) => `${Number(v).toFixed(0)}`} />
                <YAxis formatTick={(v) => `${Number(v).toFixed(1)}`} />
        ...
        <div className="glass-card rounded-2xl overflow-hidden">
          <div className="px-6 py-4 border-b border-border/5 flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] text-muted-foreground/35 mt-0.5 font-light">
                {tableData.length} components — click to inspect
              </p>
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse text-sm">
              <thead>
                <tr className="border-b border-border/5">
                  ...
                  <th className="px-6 py-3 text-[9px] font-medium text-muted-foreground/35 uppercase tracking-widest">Leakage</th>
                  <th className="px-6 py-3 text-[9px] font-medium text-muted-foreground/35 uppercase tracking-widest">Delay</th>
                  <th className="px-6 py-3"></th>
                </tr>
              </thead>
              <tbody>
                {(filter === "all" ? tableData.slice(0, 15) : tableData.slice(0, 20)).map((c: any, i: number) => (
                  <motion.tr
                    key={c.component_id}
                  ...
                  >
                    ...
                    <td className="px-6 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-muted-foreground/35 font-light tabular-nums">{c.leakage_median?.toFixed(1)}</span>
                      </div>
                    </td>
                    <td className="px-6 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-muted-foreground/35 font-light tabular-nums">{c.delay_median?.toFixed(1)}</span>
                      </div>
                    </td>
          ...
          {tableData.length > (filter === "all" ? 15 : 20) && (
